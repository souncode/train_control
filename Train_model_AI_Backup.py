...
def export_misclassified_samples(
    ...
    for stem in sorted(set(gt_labels_by_stem) | set(pred_labels_by_stem)):
        ...
        try:
            val_image_rel_path = image_file.relative_to(val_image_dir).as_posix()
        except Exception:
            val_image_rel_path = image_file.name

        try:
            gt_label_file = gt_labels_by_stem.get(stem)
            gt_label_rel_path = gt_label_file.relative_to(val_label_dir).as_posix() if gt_label_file else ""
        except Exception:
            gt_label_rel_path = ""
            ...
            for gt_id in missing_gt:
                for pred_id in extra_pred[:5]:
                    sample_rows.append({
                        ...
                        "val_image_rel_path": val_image_rel_path,
                        "gt_label_rel_path": gt_label_rel_path,
...
def next_train_run_name(output_root: Path, base_name: str = "Model_Train") -> str:
    if not (output_root / base_name).exists():
        return base_name

    idx = 2
    while (output_root / f"{base_name}{idx}").exists():
        idx += 1
    return f"{base_name}{idx}"


def main():
    current_project_path = Path.cwd().resolve()
    current_project_dir = str(current_project_path)
    current_project_name = current_project_path.name
    output_root = current_project_path / "Output"
    output_root.mkdir(parents=True, exist_ok=True)
    run_name = next_train_run_name(output_root)
    ...
    push_log(f"Output root: {output_root}")
    push_log(f"Run name: {run_name}")
